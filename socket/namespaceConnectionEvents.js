...
const namespaceConnectionEvents = async (namespaceSocket, namespaceID) => {
  const currentNamespace = socket.getIO().of(`/${namespaceID}`);
  ...
  namespaceSocket.emit('namespace_joined', namespaceID);
  console.log(`joined namespace ${namespaceID}`);
  ...
  namespaceSocket.emit(
    'namespace_data',
    await namespaceController.getNamespaceData(namespaceID)
  );
